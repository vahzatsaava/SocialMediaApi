...
public class MessageController {
    ...
    public ResponseEntity<ResponseDto<String>> sendMessage(Principal principal,
                                                           @RequestParam @Size(min = 8) @Email String receiverEmail, @RequestBody String content) {
        messageService.sendMessage(principal,receiverEmail,content);
        ResponseDto<String> responseDto = new ResponseDto<>(HttpStatus.OK.value(),
                String.format("Message was send from %s to %s", principal.getName(), receiverEmail));
    ...
    public ResponseEntity<ResponseDto<List<ContentChatFriendsDto>>> getFriendsChat(Principal principal,
                                                                                   @RequestParam @Email @Size(min = 8) String emailReceiver) {
        List<ContentChatFriendsDto> messageDtos = messageService.getChatBetweenUsers(principal,emailReceiver);
